    ...
    public function doInstall($command, $options, $params, $objects = array())
    {
        ...
        try {
            ...
            if ($ftp) {
                ...
                $isWritable = isWritableUOLHost($config->magento_root)
                    && isWritableUOLHost($config->magento_root . DIRECTORY_SEPARATOR . $config->downloader_path)
                    && isWritableUOLHost($config->magento_root . $dirCache)
                    && isWritableUOLHost($config->magento_root . $dirTmp)
                    && isWritableUOLHost($config->magento_root . $dirMedia);
                $err = "Please check for sufficient write file permissions.";
            }
            $isWritable = $isWritable && isWritableUOLHost($config->magento_root . $dirMedia)
                && isWritableUOLHost($config->magento_root . $dirCache)
                && isWritableUOLHost($config->magento_root . $dirTmp);
            if (!$isWritable) {
