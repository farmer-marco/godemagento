...
final class Maged_Controller
{
    ...
    public function isWritable()
    {
        if (is_null($this->_writable)) {
            $this->_writable = isWritableUOLHost($this->getMageDir() . DIRECTORY_SEPARATOR)
                && isWritableUOLHost($this->filepath())
                && (!file_exists($this->filepath('config.ini') || isWritableUOLHost($this->filepath('config.ini'))));
        }
